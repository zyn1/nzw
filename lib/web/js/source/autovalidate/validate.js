(function autoValidate()
{
    ...
    function validateOnChange()
    {
        ...
        var patternOther = textfield.getAttribute("other");
        var verifyFields = textfield.getAttribute("name");
        if(patternOther)
        {
            $.getJSON(creatUrl("/simple/verifyAbled"),{"tableName":patternOther,'value':value,'fields':verifyFields},function(res)
            {
                if(!res)
                {
                    ...
                    textfield.className=textfield.className.replace("valid-text","");
                     ...
                     if(msg && (msg.tagName=='LABEL' || msg.tagName=='SPAN'))
                     {
                        ...
                        if(textfield.getAttribute('initmsg')==null) textfield.setAttribute('initmsg', msg.innerHTML);
                         msg.innerHTML=textfield.getAttribute("otheralt");
                     }
                     else
                     {
                         ...
                         if(textfield.getAttribute('initmsg')==null) textfield.setAttribute('initmsg', '');
                          new_msg.innerHTML=textfield.getAttribute("otheralt");
        ...
        var max_value = textfield.getAttribute("max_value");
        ...
        else if(max_value && value > max_value)
        {
            ...
            textfield.className=textfield.className.replace("valid-text","");
             ...
             if(msg && (msg.tagName=='LABEL' || msg.tagName=='SPAN'))
             {
                 msg.className = "invalid-msg";
                if(textfield.getAttribute('initmsg')==null) textfield.setAttribute('initmsg', msg.innerHTML);
                 msg.innerHTML=textfield.getAttribute("maxalt");
             }
             else
             {
                 var new_msg=document.createElement("LABEL");
                 new_msg.className = "invalid-msg";
                 if(textfield.getAttribute('initmsg')==null) textfield.setAttribute('initmsg', '');
                  new_msg.innerHTML=textfield.getAttribute("maxalt");
                 textfield.parentNode.insertBefore(new_msg,msg);
             }   
        }
        else
    ...
    function validateOnSubmit()
    {
        ...
        var noComplate = this.getAttribute('noComplate');
        var result = true;
        if(callback !=null) {result = eval(callback);}
        result = !(result==undefined?true:result);
        if (invalid || result || noComplate)
        {
            return false;
        }
